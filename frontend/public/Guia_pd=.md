...
El modelo PD= sigue el Art. 100.2 del Decreto 91/2024 (Aragón), en su redacción dada por el **BOA nº 181, de 18 de septiembre de 2025**: Portada + 14 apartados con letra (A a N, con subapartados anidados en C/E/G). Aplica a Grado D (que incluye tanto Grado Medio como Grado Superior) sin distinción. A continuación se indica **exactamente dónde se rellena** cada apartado en Cuaderno FP.
...
## A. Introducción
Marco normativo, contextualización, datos del módulo/ciclo/duración y relación de unidades formativas.
- **Dónde:** `Programación` › `Contexto` → Pestaña `Identificación` (datos del módulo) y Pestaña `Contextualización` (entorno geográfico/socioeconómico/escolar) · texto override en `Programación` › `Contexto` → sección "Textos del modelo Simplificado (pd=)"
...
### C1. Contenidos
Listado de UD con su descripción.
- **Dónde:** `Programación` › `Currículo` → Pestaña `Unidades didácticas`
...
- **Dónde:** sin campo específico todavía — usa el valor por defecto ("no se imparte en modalidad bilingüe") salvo que se añada un campo dedicado.
...
## Notas
- Los apartados del currículo (C, C2, H) se rellenan automáticamente desde los datos ya introducidos para el resto de la app.
- Todos los apartados D-N tienen un texto por defecto razonable si no se rellena ningún campo específico — nunca se genera un documento con secciones vacías o con datos de un módulo distinto al tuyo.
- **Corrección 2026-09-13:** hasta esta fecha, este documento (y `Comparativa_PD.md`) describían una estructura de 17 apartados A-Q que corresponde al Art. 100.2 del Decreto 91/2024 **antes** de su modificación por el BOA nº 181/2025 — no es la estructura real de ningún documento generado por la app. Corregido tras verificar directamente los PDF reales de PD= (curso 2025/2026) y la plantilla activa (`modelo_pd_fp=.docx`), que ya usan la estructura de 14 apartados de arriba.
